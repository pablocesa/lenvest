...
class DonutProgress @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (attributeResourceId != 0) {
            val bitmap =
                BitmapFactory.decodeResource(resources, attributeResourceId)
            canvas.drawBitmap(
                bitmap,
    ...
    override fun onSaveInstanceState(): Parcelable {
        ...
        bundle.putInt(
            INSTANCE_FINISHED_STROKE_COLOR,
    ...
    override fun onRestoreInstanceState(state: Parcelable) {
        if (state is Bundle) {
            ...
            innerBottomTextSize =
                bundle.getFloat(INSTANCE_INNER_BOTTOM_TEXT_SIZE)
            innerBottomText = bundle.getString(INSTANCE_INNER_BOTTOM_TEXT)
            innerBottomTextColor =
                bundle.getInt(INSTANCE_INNER_BOTTOM_TEXT_COLOR)
            finishedStrokeColor =
                bundle.getInt(INSTANCE_FINISHED_STROKE_COLOR)
            unfinishedStrokeColor =
                bundle.getInt(INSTANCE_UNFINISHED_STROKE_COLOR)
            finishedStrokeWidth =
                bundle.getFloat(INSTANCE_FINISHED_STROKE_WIDTH)
            unfinishedStrokeWidth =
                bundle.getFloat(INSTANCE_UNFINISHED_STROKE_WIDTH)
